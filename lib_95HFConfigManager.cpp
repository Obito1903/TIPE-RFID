 ...
 * @brief   This file provides set of firmware functions to manages device
 *modes.
 ******************************************************************************
 * @attention
 *
 ...
 * 	@{
 *	@brief  <b>This is the library used by the whole 95HF family ( CR95HF )
 *<br /> You will find ISO libraries ( 14443A, 14443B, 15693, ...) for PICC and
 *PCD <br /> The libraries selected in the project will depend of the
 *application targetted <br /> and the product chosen (RX95HF emulate PICC,
 *CR95HF emulate PCD, ST95HF can do both)</b>
 */
 ...
 * 	@{
 *	@brief  This file is used to select a configuration, PICC or PCD or
 *Initiator/Target.
 */
...
/**
 *	@brief  This function set a variable to inform Manager that a task is on
 *going
 *  @param  none
 *  @retval none
 */
...
/**
 *	@brief  This function sends POR sequence. It is used to initialize chip
 *after a POR.
 *  @param  none
...
/**
 *	@brief  This interface function inform Manager that current task must be
 *stopped
 *  @param  none
 *  @retval none
 */
void ConfigManager_Stop(void) { StopProcess = true; }

/**
 * @brief  This function initialize the NFC chip
 * @brief  Physical communication with chip enabled, RF communication not
 * enabled
 * @param  None
 * @retval None
 */
void ConfigManager_HWInit(void) {
  ...
  if (ConfigManager_PORsequence() != MANAGER_SUCCESSCODE) {
    /* nothing to do, this is a trap for debug purpose you can use it to detect
     * HW issue */
 ...
 * @param  	tagsToFind : Flags to select the different kinds of tag to
 * track, same as return value
 ...
 * @retval 	TRACK_NFCTYPE5 : A ISO/IEC 15693 type A tag is present in the RF
 * field
 */
uint8_t ConfigManager_TagHunting(uint8_t tagsToFind) {
  ...
  //           (tagsToFind & TRACK_NFCTYPE2)) /* TT2 */
  //         return TRACK_NFCTYPE2;
  //       else if (((ISO14443A_Card.SAK & 0x20) != 0x00) &&
  //                (tagsToFind & TRACK_NFCTYPE4A)) /* TT4A */
