 ...
 * @brief   This file provides set of function defined into ISO15693
 *specification
 ******************************************************************************
 * @attention
 *
 ...
 * -----------------------------------------------------------------------------
 */
#define ISO15693_MAXLENGTH_INVENTORY                                           \
  13 // 8 + 8 + 8 + 64 + 16 = 104bits => 13 bytes
#define ISO15693_MAXLENGTH_STAYQUIET                                           \
  12 // 8 + 8 	 + 64 + 16 = 96 => 12 bytes
#define ISO15693_MAXLENGTH_READSINGLEBLOCK                                     \
  13 // 8 + 8 + 8 + 64 + 16 = 104bits => 13 bytes
#define ISO15693_MAXLENGTH_LOCKSINGLEBLOCK                                     \
  13 // 8 + 8 + 8 + 64 + 16 = 104bits => 13 bytes
#define ISO15693_MAXLENGTH_READMULBLOCK                                        \
  14 // 8 + 8 + 8 + 64 + 8 + 16 = 112 bits => 14 bytes
#define ISO15693_MAXLENGTH_SELECT                                              \
  12 // 8 + 8 	 + 64 + 16 = 96 bits => 12 bytes
#define ISO15693_MAXLENGTH_RESETTOREADY                                        \
  12 // 8 + 8 	 + 64 + 16 = 96 bits => 12 bytes
#define ISO15693_MAXLENGTH_WRTITEAFI                                           \
  13 // 8 + 8 + 8 + 64 + 16 = 104bits => 13 bytes
#define ISO15693_MAXLENGTH_WRTITEDSFID                                         \
  13 // 8 + 8 + 8 + 64 + 16 = 104bits => 13 bytes
#define ISO15693_MAXLENGTH_LOCKAFI                                             \
  12 // 8 + 8 	 + 64 + 16 = 96 bits => 12 bytes
#define ISO15693_MAXLENGTH_LOCKDSFID                                           \
  12 // 8 + 8 	 + 64 + 16 = 96 bits => 12 bytes
#define ISO15693_MAXLENGTH_GETMULSECURITY                                      \
  14 // 8 + 8 + 8 + 64 + 8 + 16 = 112 bits => 14 bytes
#define ISO15693_MAXLENGTH_GETSYSTEMINFO                                       \
  12 // 8 + 8 	 + 64 + 16 = 96 => 12 bytes
// reply length
#define ISO15693_MAXLENGTH_REPLYINVENTORY                                      \
  12 // 8 + 8 + 64 + 16  = 96 => 12 bytes
#define ISO15693_MAXLENGTH_REPLYSTAYQUIET 4 // No reply
#define ISO15693_MAXLENGTH_REPLYREADSINGLEBLOCK                                \
  MAX_BUFFER_SIZE // 8 + 8 + 8 + 64 + 16 = 104bits => 13 bytes
#define ISO15693_MAXLENGTH_REPLYWRITESINGLEBLOCK                               \
  4 // 8 + 8 + 16 = 32 bits => 4 bytes
#define ISO15693_MAXLENGTH_REPLYLOCKSINGLEBLOCK                                \
  4 // 8 + 8 + 16 = 32 bits => 4 bytes
...
#define ISO15693_MAXLENGTH_REPLYRESETTOREADY                                   \
  4 // 8 + 8 + 16 = 32 bits => 4 bytes
...
#define ISO15693_MAXLENGTH_REPLYGETSYSTEMINFO                                  \
  15 // 8 + 8 + 64 + 8 + 8 + 8 + 16 = 112 bits => 15 bytes
...
typedef struct {
  int8_t DataRate,     // 0 => 26k 	1 => 52 k 	2 => 6k	3=>RFU
      WaitOrSOF,       // 0 => respect 312 �s 	1=> wait for SOF
...
/* ISO15693 commands --- */
int8_t ISO15693_RunInventory16slots(const uint8_t Flags, const uint8_t AFI,
                                    uint8_t *NbTag, uint8_t *pUIDout);
int8_t ISO15693_RunAntiCollision(const uint8_t Flags, const uint8_t AFI,
                                 uint8_t *NbTag, uint8_t *pUIDout);

// Get functions
int8_t ISO15693_GetUID(uint8_t *UIDout);
int8_t ISO15693_GetTagIdentification(uint16_t *Length_Memory_TAG,
                                     uint8_t *Tag_Density, uint8_t *IC_Ref_TAG);
int8_t ISO15693_GetSystemInfo(const uint8_t Flags, const uint8_t *UIDin,
                              uint8_t *pResponse);

// Tag functions
uint8_t ISO15693_ReadBytesTagData(uint8_t Tag_Density, uint8_t IC_Ref_Tag,
                                  uint8_t *Data_To_Read,
                                  uint16_t NbBytes_To_Read,
                                  uint16_t FirstBytes_To_Read);
uint8_t ISO15693_WriteBytes_TagData(uint8_t Tag_Density, uint8_t *Data_To_Write,
                                    uint16_t NbBytes_To_Write,
                                    uint16_t FirstBytes_To_Write);

#ifdef ISO15693_ALLCOMMANDS

// int8_t ISO15693_LockSingleBlock(const uint8_t Flags, const uint8_t *UIDin,
// const uint8_t BlockNumber,uint8_t *pResponse);

// int8_t ISO15693_Select(const uint8_t Flags, const uint8_t *UIDin, const
// uint8_t AppendCRC,const uint8_t *CRC16,uint8_t *pResponse); int8_t
// ISO15693_WriteAFI(const uint8_t Flags, const uint8_t *UIDin,const uint8_t
// AFIToWrite,uint8_t *pResponse); int8_t ISO15693_LockAFI(const uint8_t Flags,
// const uint8_t *UIDin, uint8_t *pResponse); int8_t ISO15693_WriteDSFID(const
// uint8_t Flags, const uint8_t *UIDin,const uint8_t DSFIDToWrite,uint8_t
// *pResponse); int8_t ISO15693_LockDSFID(const uint8_t Flags, const uint8_t
// *UIDin, uint8_t *pResponse); int8_t
// ISO15693_GetMultipleBlockSecutityStatus(const uint8_t Flags, const uint8_t
// *UIDin, const uint8_t BlockNumber, const uint8_t NbBlocks,uint8_t
// *pResponse);
#endif /* ISO15693_ALLCOMMANDS*/

int8_t ISO15693_SplitInventoryResponse(const uint8_t *ReaderResponse,
                                       const uint8_t Length, uint8_t *Flags,
                                       uint8_t *DSFIDextract,
                                       uint8_t *UIDoutIndex);
