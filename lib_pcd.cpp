 ...
 * @brief   This file provides set of firmware functions to manages the PCD
 *device device.
 ...
 * 	@{
 *	@brief  <b>This is the library used by the whole 95HF family (RX95HF,
 *CR95HF, ST95HF) <br /> You will find ISO libraries ( 14443A, 14443B, 15693,
 *...) for PICC and PCD <br /> The libraries selected in the project will depend
 *of the application targetted <br /> and the product chosen (RX95HF emulate
 *PICC, CR95HF emulate PCD, ST95HF can do both)</b>
 */

/** @addtogroup PCD
 * 	@{
 *	@brief  This part of the library enables PCD capabilities of CR95HF &
 *ST95HF.
 */

/** @addtogroup PCD_Device
 * 	@{
 *	@brief  This set of function use the device commands to control both
 *CR95HF & ST95HF <br /> This layer is the bridge beetween the library and the
 *95HF driver.
 */
...
/**
 *	@brief  This function computes the UART baud rate according to Baudrate
 *command parameter
 *  @param  BaudRateCommandParameter  : Baudrate command parameter (1 byte)
 *  @retval the UART baud rate value
 */
static uint32_t
PCD_ComputeUARTBaudRate(const uint8_t BaudRateCommandParameter) {
  return ((uint32_t)13.56e6 / (2 * BaudRateCommandParameter + 2));
}

/**
 *	@brief  this functions returns PCD_SUCCESSCODE if the protocol is
 *available, otherwise PCD_ERRORCODE_PARAMETER
 ...
 */
int8_t PCD_ProtocolSelect(const uint8_t Length, const uint8_t Protocol,
                          const uint8_t *Parameters, uint8_t *pResponse) {
...
/**
 *	@brief  this function send a SendRecv command to the PCD device. the
 *command to contacless device is embeded in Parameters.
 ...
 */
int8_t PCD_SendRecv(const uint8_t Length, const uint8_t *Parameters,
                    uint8_t *pResponse) {
 ...
 *  @param	Flags		: whether to increment address after register
 *read
 *  @param  pResponse : pointer on the PCD device response
 *  @return PCD_SUCCESSCODE : the command was succedfully send
 *  @return PCD_ERRORCODE_DEFAULT : the PCD device returned an error code
 *  @return PCD_ERRORCODE_PARAMETERLENGTH : Length parameter is erroneous
 */
int8_t PCD_ReadRegister(const uint8_t Length, const uint8_t Address,
                        const uint8_t RegCount, const uint8_t Flags,
                        uint8_t *pResponse) {
 ...
 *  @return PCD_ERRORCODE_PARAMETERLENGTH : Length parameter is erroneous
 */
int8_t PCD_WriteRegister(const uint8_t Length, const uint8_t Address,
                         const uint8_t Flags, const uint8_t *pData,
                         uint8_t *pResponse) {
...
/**
 *	@brief  This function sends POR sequence. It can be use to initialize
 *the PCD device after a POR.
 *  @param  none
...
/**
 *	 @brief  this function send a command to the PCD device device over SPI
 *or UART bus and receive its response.
 *	 @brief  the returned value is PCD_SUCCESSCODE
 *  @param  *pCommand  : pointer on the buffer to send to the the PCD device (
 *Command | Length | Data)
 *  @param  *pResponse : pointer on the the PCD device response ( Command |
 *Length | Data)
...
/**
 * @brief  	this function returns PCD_SUCCESSCODE is the reader reply is a
 * succesful code.
...
int8_t PCD_IsCRCOk(const uint8_t Protocol, const uint8_t *pReaderReply) {
  ...
  switch (Protocol) {
  case PCD_PROTOCOL_ISO15693:
    if ((pReaderReply[PCD_DATA_OFFSET + NbByte - CONTROL_15693_NBBYTE] &
         CONTROL_15693_CRCMASK) == CONTROL_15693_CRCMASK) {
      return PCD_ERRORCODE_DEFAULT;
    } else {
      return PCD_SUCCESSCODE;
    }
    ...
    //   case PCD_PROTOCOL_ISO14443A:
    //     if ((pReaderReply[PCD_DATA_OFFSET + NbByte - ISO14443A_NBBYTE] &
    //          ISO14443A_CRCMASK) == ISO14443A_CRCMASK) {
    //       return PCD_ERRORCODE_DEFAULT;
    //     } else {
    //       return PCD_SUCCESSCODE;
    //     }
    ...
    //   case PCD_PROTOCOL_ISO14443B:
    //     if ((pReaderReply[PCD_DATA_OFFSET + NbByte - CONTROL_14443B_NBBYTE] &
    //          CONTROL_14443B_CRCMASK) == CONTROL_14443B_CRCMASK) {
    //       return PCD_ERRORCODE_DEFAULT;
    //     } else {
    //       return PCD_SUCCESSCODE;
    //     }
    ...
    //   case PCD_PROTOCOL_FELICA:
    //     if ((pReaderReply[PCD_DATA_OFFSET + NbByte - CONTROL_FELICA_NBBYTE] &
    //          CONTROL_FELICA_CRCMASK) == CONTROL_FELICA_CRCMASK) {
    //       return PCD_ERRORCODE_DEFAULT;
    //     } else {
    //       return PCD_SUCCESSCODE;
    //     }
 ...
 * @retval  	PCD_ERRORCODE_DEFAULT  :  the PCD device didn't return a
 * succesful code
...
/**
 *	@brief  this function perform the calibration to provide the parameter
 *require for tag detection command
 *  @param	WU Period	: Time allow between 2 tag detections
 *  @param	pDacDataH	: Value computed for DacDataH (DacDataL =
 *DacDataH � 0x10)
 *  @return PCD_SUCCESSCODE : the calibration was successfully performed
 *(DacDataH value is significant)
 */
int8_t PCD_TagDetectCalibration(const uint8_t Wuperiod, uint8_t *pDacDataH) {
  uint8_t DataToSend[IDLE_CMD_LENTH + 2] = {0x00, 0x00, 0x03, 0xA1, 0x00, 0xF8,
                                            0x01, 0x18, 0x00, 0x00, 0x60, 0x60,
                                            0x00, 0x00, 0x3F, 0x01};
  ...
  /* During the calibration process, DacDataL is forced to 0x00 and the software
     successively
           varies the DacDataH value from its maximum value (0xFE) to its
     minimum value (0x00) */
  ...
  if ((Response[0] == 0x00) && (Response[1] == 0x01) && (Response[2] == 0x02)) {
    ...
    if ((Response[0] == 0x00) && (Response[1] == 0x01) &&
      ...
      /* If last Wake-up event = Tag Detect (0x02), search DacDataRef = last
         DacDataH value If last Wake-up event = Timeout (0x01), search
         DacDataRef = last DacDataH value -4 */
      /* We have founded DacDataRef, DacDataL= DacDataRef � Guard and DacDataH=
         DacDataRef+ Guard where guard = 0x08 (2 DAC steps) */
      if ((Response[0] == 0x00) && (Response[1] == 0x01) &&
          (Response[2] == 0x01))
 ...
 *  @param	WU Source	: Condition for exiting this function, Condition
 *that was realized
 *  @param	WU Period	: Time allow between 2 tag detections
 *  @param	DacDataH 	: value that must have been computed by calling
 *PCD_TagDetectCalibration function
 *  @param	NbTrials	: number of trial to find tag before time out
 *  @return PCD_SUCCESSCODE : the calibration was succesfully performed
 *(DacDataH value is significant)
 */
int8_t PCD_WaitforTagDetection(uint8_t *WuSource, const uint8_t WuPeriod,
                               const uint8_t DacDataH, const uint8_t NbTrials) {
  uint8_t DataToSend[IDLE_CMD_LENTH + 2] = {0x00, 0x00, 0x00, 0x21, 0x00, 0x79,
                                            0x01, 0x18, 0x00, 0x20, 0x60, 0x60,
                                            0x00, 0x00, 0x3F, 0x01};
  uint8_t Response[3] = {0x00};
...
/**
 *	@brief  This function run a process to change UARt baud rate. It'll
 *check the baud rate is
 *	@brief   compatible with this MCU and configurates MCU and the PCD
 *device baud rate.
 *  @param  *pCommand  : pointer on the buffer to send to the the PCD device (
 *Command | Length | Data)
 *  @param  *pResponse : pointer on the the PCD device response ( Command |
 ...
 *  @retval  PCD_ERRORCODE_UARTDATARATEUNCHANGED : the UART baud rate has not
 *changed
 *  @retval  PCD_ERRORCODE_UARTDATARATEPROCESS : command DaubRate was send to
 *the PCD device but the MCU was not able to comunicatewith the PCD device
 */
