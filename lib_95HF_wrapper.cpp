 ...
 * 	@{
 *	@brief  <b>This is the library used to manage the content of the TAG
 *(data) But also the specific feature of the tag, for instance password, gpo...
 *</b>
 */
...
uint16_t ReadData(uint16_t Offset, uint16_t DataSize, uint8_t *pData) {
  ...
  switch (nfc_tagtype) {
    ...
    //       if (TT2Tag[NFCT2_MAX_CONFIG + NFCT2_MAX_CC_SIZE + 1] == 0xFF)
    //         pData[i - Offset] =
    //             TT2Tag[NFCT2_MAX_CONFIG + NFCT2_MAX_CC_SIZE + 2 + i];
    //       /* 1 bytes length */
    //       else
    //         pData[i - Offset] = TT2Tag[NFCT2_MAX_CONFIG + NFCT2_MAX_CC_SIZE +
    //         i];
    //     }
    //   }
    //   break;
