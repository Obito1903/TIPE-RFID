...
/**
 *  @brief This uTimeOut variable is used as a timeout duting the communication
 * with the RF tranceiver
 */
...
/**
 *	@brief  this functions initializes the SPI in order to communicate with
 *the 95HF device
 *  @param  None
 *  @retval void
 */
static void drv95HF_InitializeSPI(void) { RFTRANS_SPI_Init(); }
...
/**
 *	@brief  Send a reset sequence over SPI bus (Reset command ,wait
 *,negative pulse on IRQin).
 *  @param  None
 *  @retval None
 */
...
//{
//  if (HAL_GPIO_ReadPin(IRQOUT_RFTRANS_95HF_PORT, IRQOUT_RFTRANS_95HF_PIN) !=
//  GPIO_PIN_RESET)
//  {
//    return 0x01;
//  }
...
/**
 *	@brief  This function initialize MCU serial interface peripheral (SPI or
 *UART)
 *  @param  None
 *  @retval None
 */
 ...
 */
uint8_t drv95HF_GetSerialInterface(void) { return drv95HFConfig.uInterface; }
...
void drv95HF_SendSPICommand(const uint8_t *pData) {
  ...
  if (*pData == ECHO) {
    ...
    SPI_SendReceiveBuffer(
        pData, pData[RFTRANS_95HF_LENGTH_OFFSET] + RFTRANS_95HF_DATA_OFFSET,
        DummyBuffer);
  }
...
void drv95HF_ReceiveSPIResponse(uint8_t *pData) {
  ...
  if (pData[RFTRANS_95HF_COMMAND_OFFSET] == ECHO) {
    ...
    /* In case we were in listen mode error code cancelled by user (0x85 0x00)
     * must be retrieved */
    ...
    if (pData[RFTRANS_95HF_LENGTH_OFFSET] != 0x00)
      SPI_SendReceiveBuffer(DummyBuffer, pData[RFTRANS_95HF_LENGTH_OFFSET],
                            &pData[RFTRANS_95HF_DATA_OFFSET]);
  }
 ...
 *  @param  *pCommand  : pointer on the buffer to send to the 95HF device (
 *Command | Length | Data)
 *  @param  *pResponse : pointer on the 95HF device response ( Command | Length
 *| Data)
...
/**
 *	@brief  This function send a command to 95HF device over SPI or UART bus
 *  @param  *pCommand  : pointer on the buffer to send to the 95HF ( Command |
 *Length | Data)
 *  @retval None
 */
...
/**
 *	@brief  This function is a specific command. It's made polling and
 *reading sequence.
 *  @param  *pResponse : pointer on the 95HF device response ( Command | Length
 *| Data)
 *  @retval RFTRANS_95HF_SUCCESS_CODE : the function is succesful
 *  @retval RFTRANS_95HF_POLLING_RFTRANS_95HF : the polling sequence returns an
 *error
 */
 ...
 *  @param  WU_source : Source selected to wake up the device
 *(WU_TIMEOUT,WU_TAG,WU_FIELD,WU_IRQ,WU_SPI)
 *  @param  mode : Can be IDLE_SLEEP_MODE or IDLE_HIBERNATE_MODE
 *  @retval None
 */
void drv95HF_Idle(const uint8_t WU_source, const uint8_t mode) {
  uint8_t pCommand[] = {RFTRANS_95HF_COMMAND_IDLE,
                        IDLE_CMD_LENTH,
                        0,
                        0,
                        0,
