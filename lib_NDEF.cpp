 ...
 * @{
 * @brief  This part of the library manage data which follow NFC forum
 * organisation.
 */
...
sRecordInfo_t *SPRecordStructAdd[SP_MAX_RECORD] = {&SPRecordStruct1,
                                                   &SPRecordStruct2};
...
static uint16_t NDEF_IdentifySPRecord(sRecordInfo_t *pRecordStruct,
  ...
  uint16_t SizeOfRecordHeader, TypeNbByte, PayloadLengthField, IDLengthField,
      IDNbByte;
  ...
  SizeOfRecordHeader = RECORD_FLAG_FIELD + TYPE_LENGTH_FIELD +
                       PayloadLengthField + IDLengthField + TypeNbByte +
                       IDNbByte;
...
/**
 * @brief  This function parse the record header and dispatch regarding TNF
 * value.
 * @param  pRecordStruct : pointer on the record structure to fill.
...
static void NDEF_ParseWellKnownType(sRecordInfo_t *pRecordStruct) {
  ...
  if (!memcmp(&(pRecordStruct->Type), SMART_POSTER_TYPE_STRING,
              pRecordStruct->TypeLength)) {
    /* special case where	we have to parse others records */
    pRecordStruct->NDEF_Type = SMARTPOSTER_TYPE;
    NDEF_ParseSP(pRecordStruct);
  }

  else if (!memcmp(&(pRecordStruct->Type), URI_TYPE_STRING,
                   pRecordStruct->TypeLength)) {
  ...
  else if (!memcmp(&(pRecordStruct->Type), TEXT_TYPE_STRING,
                   pRecordStruct->TypeLength)) {
 ...
 */
static void NDEF_ParseMediaType(sRecordInfo_t *pRecordStruct) {
  if (!memcmp(&(pRecordStruct->Type), VCARD_TYPE_STRING,
              pRecordStruct->TypeLength))
    pRecordStruct->NDEF_Type = VCARD_TYPE;
  else if (!memcmp(&(pRecordStruct->Type), XVCARD_TYPE_STRING,
                   pRecordStruct->TypeLength))
    pRecordStruct->NDEF_Type = VCARD_TYPE;
  else if (!memcmp(&(pRecordStruct->Type), XVCARD2_TYPE_STRING,
                   pRecordStruct->TypeLength))
    pRecordStruct->NDEF_Type = VCARD_TYPE;
  else
    pRecordStruct->NDEF_Type = UNKNOWN_TYPE;
 ...
 */
static void NDEF_ParseForumExternalType(sRecordInfo_t *pRecordStruct) {
  if (!memcmp(&(pRecordStruct->Type), M24SR_DISCOVERY_APP_STRING,
              pRecordStruct->TypeLength))
...
static void NDEF_ParseSP(sRecordInfo_t *pRecordStruct) {
  ...
  PayloadSize = ((uint32_t)(pRecordStruct->PayloadLength3) << 24) |
                ((uint32_t)(pRecordStruct->PayloadLength2) << 16) |
                ((uint32_t)(pRecordStruct->PayloadLength1) << 8) |
  ...
  do {
    ...
    if (NDEF_IdentifySPRecord(pSPRecordStruct, pPayload) == NDEF_OK) {
       ...
       */
      SPPayloadSize = ((uint32_t)(pSPRecordStruct->PayloadLength3) << 24) |
                      ((uint32_t)(pSPRecordStruct->PayloadLength2) << 16) |
                      ((uint32_t)(pSPRecordStruct->PayloadLength1) << 8) |
                      pSPRecordStruct->PayloadLength0;
    ...
    RecordPosition++;
  } while ((OffsetInSPPayload < PayloadSize) &&
           RecordPosition < SP_MAX_RECORD); /* there is another record */
}
...
uint16_t NDEF_IdentifyNDEF(sRecordInfo_t *pRecordStruct, uint8_t *pNDEF) {
  ...
  /* read Payload */
  status =
      ReadData((uint16_t)((FIRST_RECORD_OFFSET) + pRecordStruct->PayloadOffset),
               PayloadSize, pNDEF);
...
uint16_t NDEF_IdentifyBuffer(sRecordInfo_t *pRecordStruct, uint8_t *pNDEF) {
  ...
  pRecordStruct->PayloadBufferAdd =
      (uint32_t)(&pNDEF[pRecordStruct->PayloadOffset]);
